...
public interface ProductRepository extends JpaRepository<ProductEntity, Long> {
  ...
  // 쿼리 메소드 생성법
  // 리턴 타입 + {주제 키워드 + 서술어}를 조합하여 메소드 생성 ({} 부분은 반복사용됨)
  // List<Person> findByLastnameAndEmail(String lastName, String email);

  // 쿼리 메소드의 주제 키워드
  // find ... By : select 명령을 수행하는 키워드. read/get/query/search/stream ... By 추가로 존재함.
  Optional<ProductEntity> findByNumber(Long number); // entity를 이미 지정했기 때문에 생략가능
  Optional<ProductEntity> findProductEntityByNumber(Long number); // 기본 방식
  List<ProductEntity> findAllByName(String name); // 이름을 기준으로 모든 데이터 가져오기
  ...
  // delete ... By, remove ... By : delete 명령을 수행하는 키워드. delete By는 리턴이 없음. remove By는 삭제 수를 반환
  ...
  // (Is)Not : 값의 불일치를 조건으로 사용하는 조건자 키워드. Is를 생략할 수 있음
...
//  List<ProductEntity> findByUpdatedAtIsNotNull();
  ...
  // (Is)True, (Is)False : 논리형 데이터 타입으로 지정된 컬럼값을 확인하는 키워드
  ...
  // GreaterThan, (Is)LessThan, (Is)Between : 숫자나 datetime 컬럼을 대상으로 비교 연산에 사용할 수 있는 조건자 키워드
  // GreaterThan, LessThan은 비교 대상에 대한 초과 및 미만을 비교 연산 수행
  ...
  List<ProductEntity> findByPriceGreaterThanEqual(Long price); // 지정한 가격 이상 데이터 조회
  List<ProductEntity> findByPriceIsLessThan(Long price); // 지정한 가격 미만 데이터 조회
  List<ProductEntity> findByPriceLessThan(Long price);
  List<ProductEntity> findByPriceLessThanEqual(Long price); // 지정한 가격 이하 데이터 조회
  List<ProductEntity> findByPriceIsBetween(Long lowPrice, Long highPrice); // 지정한 낮은 데이터부터 높은 데이터까지의 데이터 조회
  ...
  // OrderBy ... ASC|DESC : 쿼리문의 결과를 지정한 컬럼으로 정렬하는 키워드
  List<ProductEntity> findByNameOrderByNumberAsc(String name); // 지정한 이름으로 데이터를 조회 후 번호 기준으로 오름차순 정렬
  List<ProductEntity> findByNameOrderByNumberDesc(String name); // 지정한 이름으로 데이터를 조회 후 번호 기준으로 내림차순 정렬
  List<ProductEntity> findByNameOrderByPriceAscStockDesc(String name); // 지정한 이름으로 데이터를 조회 후 가격 기준으로 오름차순 정렬을 먼저하고 재고 수량을 기준으로 내림차순 정렬

  // 정렬 사용시 Sort객체를 사용하여 정렬하는 것이 가능함
  ...
  // JPQL 문법을 사용하여 SQL 쿼리를 생성하여 복잡한 쿼리문을 실행할 수 있도록 도와줌
  // 검색 조건에 '?순번' 형태로 데이터를 사용함
  // 사용법
  // @Query("select entitiy 객체명 from entity클래스명 where 검색조건")
  ...
  // SELECT 절에 클래스의 별명.컬럼명으로 출력하고자 하자는 컬럼만 입력
  ...
  // 메소드의 매개변수를 쿼리문에 적용할 경우 :매개변수명 형식으로 입력하는 방식이 있음
