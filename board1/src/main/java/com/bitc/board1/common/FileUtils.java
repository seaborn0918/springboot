...
// @Bean과 @Component는 동일하게 스프링 프레임워크에서 객체를 생성하여 사용하고 관리하는 라이브러리라는 의미의 어노테이션
// @Bean : 스프링 프레임워크 및 각종 서드 파티회사에서 제공하는 미리 생성해놓은 라이브러리
...
public class FileUtils {
  ...
  public List<BoardFileDTO> parseFileInfo(int boardIdx, MultipartHttpServletRequest uploadFiles) throws Exception {
    // 스프링 프레임워크가 지원하는 ObjectUtils를 사용하여 업로드된 파일이 있는지 확인. 없으면 true
    ...
    // 파일 저장 경로 생성. 현재 타임존 기준 시간을 패턴에 맞게 설정하여 출력
    ...
    String contentType; // 파일의 확장자명 저장할 변수
    ...
    while (iterator.hasNext()) {
      ...
      for (MultipartFile multipartFile : fileLists) {
        ...
        if (multipartFile.isEmpty() == false) {
          ...
          // 현재 시간을 기준으로 새 파일명 생성. nanoTime()을 사용하여 동일한 이름이 나올 수 없도록 함. 위에서 생성한 확장자명과 문자열을 연결
          ...
          // 서버에 저장되는 파일 이름. 위에서 생성한 파일 저장 결로와 nanoTime()을 이용하여 생성한 파일이름을 합하여 파일을 저장할 전체 경로를 생성
          boardFile.setStoredFileName(path + "/" + newFileName);

          // 위에서 생성한 List<BoardFileDTO> 타입의 변수에 데이터 추가
          fileList.add(boardFile);

          // 자바의 File 클래스 객체를 방금 생성한 파일이름 및 경로로 생성
